...
Start the HTTP Compose stack (with or without the telemetry overlay), build the current server image and create synthetic records with `node dev/smoke.mjs`. Then run from the repository root with Node 24 and Docker:

```sh
node --test dev/recovery.test.mjs
```

Run without concurrent application writes or key rotation, and avoid credential validity boundaries during the comparison. PostgreSQL's dump is consistent during concurrent writes, but the rehearsal compares it with separately collected table fingerprints. The rehearsal is for the local HTTP Compose deployment, not an arbitrary production database. It probes the public achievements API, so moving management endpoints to a separate port does not prevent startup detection. Its in-memory buffers are limited to 64 MiB; larger backups require a streamed operational procedure.

The check reads the running database, creates a custom-format dump, restores it into a separate PostgreSQL container and compares all public table rows, including evidence, review history, credentials, revocation, sharing, enrollments, outbox and migration history. Empty/corrupt input and restoring over populated tables must fail. It copies `/run/secrets` into a temporary volume, boots the same server image against the restored database, and compares each credential's verification status and sharing availability with the live source. Valid signatures must retain their lifecycle status; pre-existing invalid proofs are counted explicitly and must remain unchanged. At least one source credential must have a verifiable signature. A successful restore does not mean that a pre-existing invalid credential became valid: the issuer must review those findings separately and explicitly reissue affected credentials when appropriate. Existing signed documents are never rewritten by this rehearsal. Existing retained public keys are copied with the active key, preserving old-key verification when present. A second temporary server uses an unrelated signing key and no retained keys: all signatures must fail there, and the before/after comparison must detect the loss of verification trust.
