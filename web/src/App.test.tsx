...
it('lets visitors inspect sample criteria without authentication or API requests', async () => {
  useSession.setState({ authenticated: false })
  const user = userEvent.setup()
  mount()
  await user.click(screen.getByRole('link', { name: /Explore achievements/ }))
  await user.click(screen.getByRole('button', { name: 'View sample: Accessible documents' }))
  expect(screen.getByRole('dialog')).toHaveTextContent('Sample evidence')
