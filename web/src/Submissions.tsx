...
export function Submissions({ achievements }: { achievements: Achievement[] }) {
  ...
  return <section className="container workspace" aria-labelledby="workspace-title">
    ...
    {authenticated && selected && (editing ? <EvidenceForm achievement={achievement(selected.submission.achievementId)} existing={selected} onClose={close} onSubmitted={() => setOffset(0)}/> : <ReviewDetails item={selected} achievement={achievement(selected.submission.achievementId)} reviewer={reviewer} onClose={close} onEdit={() => setEditing(true)}/>)}
  </section>
}
